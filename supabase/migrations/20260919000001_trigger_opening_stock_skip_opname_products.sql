-- Products that have already gone through a confirmed stock opname should
-- never receive another placeholder "Opening Stock" batch, even if their
-- opname-counted quantity left them with no "live" stock_batches rows —
-- the opname result is the source of truth from that point on.
...
begin
  ...
  where p.base_price > 1
    ...
    and not exists (
      ...
      where soi.product_id = p.id and sos.status = 'confirmed'
    );
